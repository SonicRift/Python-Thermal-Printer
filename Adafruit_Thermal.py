...
class Adafruit_Thermal(Serial):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.writeToStdout is False:
            ...
            self.writeBytes(
                18, 35, (printBreakTime << 5) | printDensity  # DC2  # Print density
            )
        ...
        for i in range(len(data)):
            ...
            if c != 0x13:
                ...
                if (c == "\n") or (self.column == self.maxColumn):
                    ...
                    if self.prevByte == "\n":
                        # Feed line (blank)
                        d += (self.charHeight + self.lineSpacing) * self.dotFeedTime
                    else:
                        # Text line
                        d += (self.charHeight * self.dotPrintTime) + (
                            self.lineSpacing * self.dotFeedTime
                        )
                        self.column = 0
    ...
    def testPage(self):
        ...
        self.timeoutSet(self.dotPrintTime * 24 * 26 + self.dotFeedTime * (6 * 26 + 30))
        ...
        for rowStart in range(0, h, maxChunkHeight):
            ...
            for y in range(chunkHeight):
                for x in range(rowBytesClipped):
                    if self.writeToStdout:
                        sys.stdout.buffer.write(bytes([bitmap[i]]))
                    else:
                        super(Adafruit_Thermal, self).write(bytes([bitmap[i]]))
                    i += 1
